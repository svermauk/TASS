...
REAL*8, ALLOCATABLE :: cv3(:), vbias(:), ct(:)
INTEGER :: mtd_steps, md_steps, i, t_min, t_max
INTEGER :: mtd_max, w_cv, w_hill, i_mtd, i_md, nbin4
INTEGER :: i_s1, i_s2, i_s3, i_s4, nbin1, nbin2, nbin3
...
OPEN(2,FILE='Pu.dat',STATUS='replace',form='unformatted')
...
ALLOCATE(cv3(md_steps))
ALLOCATE(vbias(md_steps),ct(mtd_steps))

DO i_md=1,md_steps
 !            dum, phi1( mtd), psi1, phi2 (US), psi2, phi3, psi3, phi4, psi4
 READ(11,*) dum, cv1(i_md),dum,cv2(i_md),cv3(i_md),dum,dum,dum,dum
         !         phi1, phi2, psi2
...
DO i_md=1,md_steps
 ...
 index3 = nint((cv3(i_md)-gridmin3)/griddiff3) +1
     ...
     IF(index1.gt.0.and.index2.gt.0.and.index3.gt.0.and.&
       index1.le.nbin1.and.index2.le.nbin2.and.index3.le.nbin3) then
     
      i_mtd=((i_md-1)*w_cv/w_hill)+1
      dum=vbias(i_md) - ct(i_mtd)
      prob(index1,index2,index3) = prob(index1,index2,index3) + DEXP(dum/kt)
      den=den+DEXP(dum/kt)
...
END DO
  ...
  den=den*griddiff1*griddiff2*griddiff3
...
DO i_s1=1,nbin1
   DO i_s2=1,nbin2
      DO i_s3=1,nbin3
             ...
             prob(i_s1,i_s2,i_s3)=prob(i_s1,i_s2,i_s3)*(1.d0/den)
          
             WRITE(2)prob(i_s1,i_s2,i_s3)
         
      END DO
   END DO
END DO
      WRITE(*,'(A)')'Unbiased distribution written in PROB_3D.dat'
