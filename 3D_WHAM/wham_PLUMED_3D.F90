...
program fes_calc
implicit none
integer i, iter, ncv, umbr_n, nbin1, nbin2, nbin3
real*8, allocatable :: grid0(:,:), v(:), prob(:,:,:),biased_prob(:,:,:,:),grid(:,:)
real*8 :: kt, toler, dummy
logical :: parent
integer :: i_umbr, i_s1, i_s2, i_s3
...
nbin3=nint((grid0(2,3)-grid0(1,3))/grid0(3,3))+1
...
allocate(biased_prob(nbin1,nbin2,nbin3,umbr_n))
...
!       umbr_mean(i_umbr)=umbr_mean(i_umbr)*3.14/180 !radian conversion
      ...
      f1 = cvfile 
      open( unit=3, file=f1,form='unformatted', status='old' )
      do i_s1=1,nbin1 !US
         do i_s2=1,nbin2 !MTD
            do i_s3=1,nbin3 !TAMD
               read(3)biased_prob(i_s1,i_s2,i_s3,i_umbr)
            end do
         end do
      end do
   enddo
...
call RBcast(biased_prob,nbin1*nbin2*nbin3*umbr_n)

!do i=1,umbr_n
!call RBcast(biased_prob(1,1,1,1,i),nbin1*nbin2*nbin3*nbin4*1)
!end do

!PERFORMS WHAM.
 if (parent)  write(*,*) 'wham begins'
 CALL DistributeGrids(ncv,grid0,grid,rank,gleng1_min,gleng1_max)
 if (parent)  write(*,*) 'wham begins1'

 gleng2=nint((grid(2,2)-grid(1,2))/grid(3,2))+1
 gleng3=nint((grid(2,3)-grid(1,3))/grid(3,3))+1

 write(*,*)'new_grid', gleng1_min, gleng1_max,gleng2, gleng3, rank, grid(1,1)
  iter=0
  scf_loop : do
       iter=iter+1
       call wham_scf(a,umbr_k,umbr_mean,nmax,grid0,biased_prob,umbr_n,nbin1,nbin2,nbin3,ncv,kt,prob,cnvg,gleng1_min,gleng1_max)
       if (parent) write(*,*) 'iteration #', iter, 'convergence =',cnvg
       if (mod(iter,100) .eq. 0 ) then
       call print_pmf(ncv,nbin1,nbin2,nbin3,grid0,kt,prob,parent)
       endif
       if(cnvg.lt.toler)then
       if (parent) write(*,*)'** convergence achieved **'
       exit scf_loop
          else if(iter.gt.9999) then
...
!***********************************************!
 ...
 subroutine print_pmf(ncv,nbin1,nbin2,nbin3,grid0,kt,prob,parent)
!prints pmf 
 implicit none
 integer:: i_s1, i_s2, i_s3
 integer:: nbin1, nbin2, nbin3, ncv
 integer:: l, il, ix, iy, iz, k
 real*8 :: prob(nbin1,nbin2,nbin3), grid0(3,ncv)
 real*8 :: s1, s2, s3, kt, dum, cube_tmp(6)
 logical:: parent
 real*8, allocatable :: prob1(:,:,:),prob_b(:,:)
 character (len=50):: f2
 
 allocate(prob1(nbin1,nbin2,nbin3))
 allocate(prob_b(nbin1,nbin3))
 prob1=0.0
 CALL GlobSumR(prob,prob1,nbin1*nbin2*nbin3)
 if (parent)then
 f2= '3D_PROB'
 open( unit =7 , file = f2, status =  'unknown', form ='unformatted' )
DO i_s1=1,nbin1 !US cv
   s1=DFLOAT(i_s1-1)*grid0(3,1)+grid0(1,1)
   DO i_s2=1,nbin2
        s2=DFLOAT(i_s2-1)*grid0(3,2)+grid0(1,2)
        DO i_s3=1,nbin3
             s3=DFLOAT(i_s3-1)*grid0(3,3)+grid0(1,3)
                 write(7) prob1(i_s1,i_s2,i_s3)
            END DO  
      END DO     
  END DO

 write(*,*) '3D probability is written in 3D_PROB'
 close(7)
! write(*,*) 'NBINS', nbin3,nbin4,nbin1,nbin2
 end if
 endsubroutine
!**************************************************************************************************!
...
subroutine wham_scf(a,umbr_k,umbr_mean,nmax,grid0,biased_prob,umbr_n,nbin1,nbin2,nbin3,ncv,kt,prob,cnvg,gleng1_min,gleng1_max)
!performs wham scf.
implicit none
integer:: i_s1, i_s2, i_umbr, nbin1, nbin2, umbr_n,ncv, i_s3, nbin3
integer:: nmax(umbr_n) 
real*8 :: umbr_k(umbr_n), umbr_mean(umbr_n),dum
real*8 :: num, den, dummy_v, dummy_s1, avg, del_s1, kt, dummy, cnvg
real*8 :: prob(nbin1,nbin2,nbin3),a(umbr_n)
real*8 :: grid0(3,ncv), biased_prob(nbin1,nbin2,nbin3,umbr_n),grid(3,ncv)
integer:: rank, gleng1_min,gleng1_max,ngrid!,gleng2,gleng3,gleng4
...
do i_s3 =1,nbin3 ! TAMD
   ...
   do i_umbr=1,umbr_n
      ...
      num=num+dfloat(nmax(i_umbr))*biased_prob(i_s1,i_s2,i_s3,i_umbr)
      den=den+dfloat(nmax(i_umbr))*a(i_umbr)*dummy_v
   enddo

   prob(i_s1,i_s2,i_s3)=num/den
   if(prob(i_s1,i_s2,i_s3).ne.prob(i_s1,i_s2,i_s3)) prob(i_s1,i_s2,i_s3)=1.0D-16 !remove NaN
   if(prob(i_s1,i_s2,i_s3)+1.eq.prob(i_s1,i_s2,i_s3)) prob(i_s1,i_s2,i_s3)=1.0D-16 !remove infinity

!calculate a.
      dum=grid0(3,1)*grid0(3,2)*grid0(3,3)
   do i_umbr=1,umbr_n
      del_s1=dummy_s1 - umbr_mean(i_umbr)
      if ( del_s1 .ge. 3.14d0 ) del_s1 = 6.28d0 - del_s1
      if ( del_s1 .le.-3.14d0 ) del_s1 = 6.28d0 + del_s1
      dummy_v=dexp(-(0.50d0*umbr_k(i_umbr)*del_s1*del_s1/kt))
      dummy_a(i_umbr)=dummy_a(i_umbr) +dum*dummy_v*prob(i_s1,i_s2,i_s3)
   enddo
!enddo !end of TAMD cv loop 
enddo !end of TAMD cv loop
...
!
INTEGER :: i,ncpu,icpu,ngrids,ngrids_m,ngrids_y,ngrids_z,ngrids_o,ngrids_k
...
!ELSE IF(ncv.EQ.4)THEN
! ngrids_y=(nint((grid0(2,2)-grid0(1,2))/grid0(3,2))+1)
! ngrids_z=(nint((grid0(2,3)-grid0(1,3))/grid0(3,3))+1)
! ngrids_k=(nint((grid0(2,4)-grid0(1,4))/grid0(3,4))+1)
END IF
